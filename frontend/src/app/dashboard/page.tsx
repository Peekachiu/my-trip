    ...
    return (
        <div className="p-4 min-h-screen">
            ...
            <div className="grid gap-4">
                {trips.length === 0 ? (
                    <div className="text-center p-8 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                        <p className="text-gray-500 dark:text-gray-400">{t('dashboard.noTrips')}</p>
                        <p className="text-sm text-brand-magenta mt-1">{t('dashboard.contactAdmin')}</p>
                    </div>
                ) : (
                    trips.map(t => (
                        <Link key={t.id} href={`/trip/${t.id}`}>
                            <div className="group relative glass-card p-5 hover:shadow-lg transition-all hover:-translate-y-1 hover:border-brand-cyan/40">
                                ...
                                <div className="mt-3 flex items-center text-sm text-gray-500 dark:text-gray-400">
                                    <span className="bg-gray-100 dark:bg-gray-800 dark:text-gray-300 px-2 py-1 rounded text-xs uppercase tracking-wide">
